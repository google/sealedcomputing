...
WASM_EXPORT int start() {
  ...
  biOpaqueP256PrivateKey priv_key = biGenP256PrivateKey();
  biOpaqueP256PublicKey pub_key = biP256PublicKeyFromPrivateKey(priv_key);
  biP256PublicKeyToBin(pub_key, bad_ptr);
  return 0;
}
